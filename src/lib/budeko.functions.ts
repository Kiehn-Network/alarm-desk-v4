  ...
  .handler(async ({ data, context }) => {
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;color:#222;max-width:600px;margin:auto;padding:24px">
        ...
        <p>Guten Tag,</p>
        <p>anbei finden Sie den Bericht zum Budeko-Einsatz als PDF.</p>
    ...
    const FROM = `Budeko <bericht@${SENDER_DOMAIN}>`;
    const messageId = crypto.randomUUID();
    const idempotencyKey = `budeko-bericht-${data.id}-${Date.now()}`;
