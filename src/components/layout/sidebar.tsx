...
import {
  ...
  Receipt, Upload, HelpCircle, Rocket, Search as SearchIcon, Mail, Activity, BarChart3, RefreshCw, LifeBuoy, Map as MapIcon, ExternalLink, DoorOpen, CarFront, Contact,
} from "lucide-react";
...
const sections: Section[] = [
  ...
  { label: "Menü", items: [
    ...
    { to: "/kundenakte", label: "Kundenakte", icon: Contact, roles: ["admin", "dispatcher"] },
  ...
  { label: "Notdienste", items: [
    ...
    { to: "/notdienst/lutz/pisaweb", label: "Lutz · PisaWeb", icon: ExternalLink, module: "notdienst_lutz", roles: ["admin", "dispatcher"], external: "https://desktop.pisa.lutz-aufzuege.de/" },
  ]},
                ...
                return (
                  <li key={`${it.to}:${it.tab ?? ""}`}>
                    <Link
                      ...
                      data-tour={`nav-${it.to}${it.tab ? `?tab=${it.tab}` : ""}`}
                      className={cn(
                        "group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all",
                        active
