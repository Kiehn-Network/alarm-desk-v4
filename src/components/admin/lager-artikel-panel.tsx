    ...
    <div className="space-y-6">
      ...
      <section aria-labelledby="lager-overview-heading" className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-lg border border-border bg-card p-4"><div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Meldebestand</span><PackageX className="size-4 text-amber-600" /></div><p className="mt-2 text-2xl font-semibold">{stats.low}</p><p className="text-xs text-muted-foreground">sofort prüfen</p></div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {categoryStats.length > 0 && <div className="flex flex-wrap gap-2">{categoryStats.map((item) => <button key={item.name} type="button" className="cursor-pointer" onClick={() => setCategory(item.name)}><CategoryBadge value={`${item.name} · ${item.count}`} /></button>)}</div>}
