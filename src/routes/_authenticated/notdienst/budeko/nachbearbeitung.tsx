...
function Nachbearbeitung() {
  ...
  const del = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["bk-berichte"] });
    },
  });

  async function handleSend(b: any, email: string) {
    const doc = buildBudekoPdf(b, settings?.firmenname ?? "Budeko");
    ...
    await sendFn({
      data: {
        ...
        pdf_base64: b64,
        filename: `budeko_bericht_${b.bericht_nr}.pdf`,
      },
    });
...
function EditDialog({ id, onClose }: { id: string; onClose: () => void }) {
  const qc = useQueryClient();
  const getFn = useServerFn(getBudekoBericht);
  const updFn = useServerFn(updateBudekoBericht);
  const { data: bericht, isLoading } = useQuery({
    queryKey: ["bk-bericht", id],
