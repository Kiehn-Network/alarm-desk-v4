...
import fallbackLogo from "@/assets/alarmdesk-logo.png";
...
export const Route = createFileRoute("/login/$slug")({
  ...
  component: BrandedLoginPage,
});

function BrandedLoginPage() {
  const { slug } = useParams({ from: "/login/$slug" });
  const [domain, setDomain] = useState<DomainBranding | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/public/domain/${encodeURIComponent(slug)}`)
      .then(async (r) => {
        if (!r.ok) throw new Error("not-found");
        return r.json();
      })
      .then((d) => {
        if (!d?.ok || !d?.domain) throw new Error("not-found");
        setDomain(d.domain);
      })
  ...
  return <LoginForm domain={domain} info={info} />;
}
    ...
    <div className="min-h-screen grid place-items-center bg-background p-6">
      <div className="w-full max-w-md text-center space-y-6">
        <div className="mx-auto size-14 rounded-2xl grid place-items-center bg-card/60 backdrop-blur" style={{ boxShadow: "var(--shadow-glow)" }}>
          <img src={fallbackLogo} alt="AlarmDesk" className="size-10 object-contain" />
        </div>
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Dieser Link gehört zu keiner aktiven Organisation. Bitte prüfe die
            Adresse oder wende dich an deinen Administrator.
          </p>
        </div>
      </div>
    </div>
...
function LoginForm({ domain, info }: { domain: DomainBranding; info: VersionInfo | null }) {
  ...
  const logoSrc = domain.logo_url || fallbackLogo;
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        if (!userId) throw new Error("Kein Benutzer.");

        // Domain-Zugehörigkeit prüfen (SuperAdmin = domain_id null darf jede Domäne).
        const [{ data: profile }, { data: roles }] = await Promise.all([
          supabase.from("profiles").select("domain_id").eq("id", userId).maybeSingle(),
          supabase.from("user_roles").select("role").eq("user_id", userId),
        ]);
        const isSuper = (roles ?? []).some((r) => r.role === "superadmin");
        if (!isSuper && profile?.domain_id !== domain.id) {
          await supabase.auth.signOut();
          throw new Error("Dieses Konto gehört nicht zu dieser Domäne.");
        }

        toast.success(`Willkommen bei ${domain.firmenname}!`);
        ...
        const { error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            emailRedirectTo: `${window.location.origin}/login/${domain.slug}`,
            data: { display_name: name || email.split("@")[0] },
          },
        });
        if (error) throw error;
        toast.success("Account erstellt. Bitte E-Mail bestätigen. Ein Administrator muss dich anschließend dieser Domäne zuweisen.");
      }
    } catch (err: any) {
        ...
        <div className="relative z-10 p-12 flex flex-col justify-between w-full">
          <div className="flex items-center gap-3">
            <div className="size-11 rounded-xl grid place-items-center bg-card/40 backdrop-blur overflow-hidden" style={{ boxShadow: "var(--shadow-glow)" }}>
              <img src={logoSrc} alt={domain.firmenname} className="size-8 object-contain" />
            </div>
            <div>
              <div className="text-lg font-semibold">{domain.firmenname}</div>
              <div className="text-xs text-muted-foreground">powered by AlarmDesk</div>
            </div>
          </div>
          <div>
        ...
        <div className="w-full max-w-sm">
          <div className="lg:hidden mb-8 flex items-center gap-3">
            <div className="size-10 rounded-xl grid place-items-center bg-card overflow-hidden">
              <img src={logoSrc} alt={domain.firmenname} className="size-7 object-contain" />
            </div>
            <div className="font-semibold">{domain.firmenname}</div>
          </div>

          {domain.wartung_aktiv && (
            <div className={`mb-4 rounded-lg border p-3 text-sm ${
              domain.wartung_farbe === "destructive" ? "border-destructive/40 bg-destructive/10 text-destructive" :
              domain.wartung_farbe === "warning" ? "border-warning/40 bg-warning/10 text-warning" :
              "border-info/40 bg-info/10 text-info"
            }`}>
              <div className="flex items-center gap-2 font-medium"><AlertTriangle className="size-4" /> Wartungsmodus</div>
              {domain.wartung_nachricht && <p className="mt-1 text-foreground/80">{domain.wartung_nachricht}</p>}
            </div>
          )}
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {mode === "login"
              ? <>Login für <span className="text-foreground font-medium">{domain.firmenname}</span></>
              : <>Neuer Zugang für <span className="text-foreground font-medium">{domain.firmenname}</span></>}
          </p>
